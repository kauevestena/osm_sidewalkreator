...
if QgsApplication:
    try:
        ...
        print(f"Error obtaining QGIS paths in osm_fetch: {e}. Setting basepath to current working directory.")
        basepath = "."
else:
    print("QGIS context not found, setting basepath to current working directory for osm_fetch module.")
    basepath = "."
...
def get_osm_data(querystring,tempfilesname,geomtype='LineString',print_response=False,timeout=30,return_as_string=False):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".osm", delete=False, encoding='utf-8') as tmp_osm:
            tmp_osm.write(xml_filecontent)
            temp_osm_file_path = tmp_osm.name
        
        datasource = ogr.Open(temp_osm_file_path)
        if datasource is None:
            print(f"Error: Could not open OSM data from {temp_osm_file_path} using GDAL.")
        ...
        if ogr_layer is None:
            print(f"Error: Layer '{layer_name}' not found in {temp_osm_file_path}. Available layers:")
            if datasource: # Check datasource is not None
                for i in range(datasource.GetLayerCount()):
                    lyr = datasource.GetLayer(i)
                    if lyr: print(f"  - {lyr.GetName()}")
            return None
        ...
        while ogr_feature:
            ...
            if geom:
                try:
                    ...
                    print(f"Warning: Error exporting geometry to GeoJSON for a feature: {e}")
            ...
            if 'other_tags' in properties and properties['other_tags'] is not None:
                try:
                    ...
                    if isinstance(tags_str, str) and tags_str.strip():
                        ...
                        for match in re.finditer(r'"([^"]+)"=>"((?:[^"]|"")*)"', tags_str):
                    ...
                    print(f"Warning: Could not parse 'other_tags' field content: '{properties.get('other_tags', '')}'. Error: {e}")
        ...
        if temp_osm_file_path and os.path.exists(temp_osm_file_path):
            try:
                ...
                print(f"Warning: Could not delete temporary OSM file '{temp_osm_file_path}': {e}")
    ...
    print('Conversion successful with GDAL!') # Changed print message slightly for clarity

    if return_as_string:
        return json.dumps(geojson_datadict)
    else:
        geojsonfilepath = join_to_a_outfolder(tempfilesname+'_osm.geojson')
        print('GeoJSON will be written to: ', geojsonfilepath) # Changed print message slightly
