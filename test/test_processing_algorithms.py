...
# ---------------------- Draw Missing Crossings BBOX Algorithm Tests ----------------------


def _mock_missing_crossings_osm_data(monkeypatch):
    """Mock OSM data fetching for missing crossings tests."""
    ...
    def fake_get_osm_data(**kwargs):
        ...
        if "sidewalk" in tempfilesname:
            ...
            # Mock street data for protoblocks
            geojson = json.dumps({
                "type": "FeatureCollection", 
                "features": [
                    {
...
def test_draw_missing_crossings_success(monkeypatch):
    ...
    _mock_missing_crossings_osm_data(monkeypatch)
