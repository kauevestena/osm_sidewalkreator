...
class TestOsmFetch(unittest.TestCase):
    ...
    def test_get_osm_data_linestring(self):
        ...
        try:
            ...
            self.fail(f"Returned string is not valid JSON: {e}\nString was: {geojson_str}")
        ...
        if not features: return # Avoid index error if no features
        ...
        self.assertTrue(len(geometry.get('coordinates', [])) > 0, "LineString should have coordinates.")
        ...
        self.assertNotIn('other_tags', properties, "'other_tags' field should not be present if tags are flattened.")
    ...
    def test_get_osm_data_point(self):
        ...
        self.assertEqual(len(geometry.get('coordinates', [])), 2, "Point should have 2 coordinates.")
    ...
    def test_get_osm_data_complex_tags_and_types(self):
        ...
        self.assertEqual(len(line_features), 1) # Only one way in SAMPLE_OSM_XML_COMPLEX
        ...
        self.assertEqual(line_props.get('custom:tag'), 'custom_value', "Custom tag not found or not matching.")
        self.assertEqual(line_props.get('another:tag:with:colons'), 'value:with:colon', "Tag with colons not found or not matching.")
        self.assertNotIn('other_tags', line_props, "Tags should be flattened, 'other_tags' field should not be present unless it's an actual OSM tag named 'other_tags'.")
        ...
        self.assertEqual(len(point_features), 2, f"Expected 2 point features, got {len(point_features)}") 
    ...
    def test_get_osm_data_empty_input(self):
        ...
        empty_osm_xml = """<?xml version='1.0' encoding='UTF-8'?><osm version="0.6"></osm>"""
        geojson_str = get_osm_data(
    ...
    def test_get_osm_data_unsupported_geomtype(self):
        ...
        self.assertIsNone(result, "Function should return None for unsupported geomtype.")
