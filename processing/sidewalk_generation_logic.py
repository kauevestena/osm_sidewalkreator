...
from qgis.core import (
    ...
    QgsProcessingException, Qgis, edit
)

# Assuming these are available from the main plugin structure
from ..generic_functions import (
    dissolve_tosinglegeom, get_first_feature_or_geom, generate_buffer,
    compute_difference_layer, convert_multipart_to_singleparts,
    remove_biggest_polygon, extract_lines_from_polygons,
    layer_from_featlist, geom_to_feature, create_new_layerfield,
    create_area_field, create_perimeter_field
)
from ..parameters import (
    widths_fieldname, big_buffer_d #, sidewalk_tag_value and other specific tags if needed here
)
...
def generate_sidewalk_geometries_and_zones(
    ...
    feedback.pushInfo(f"Sidewalk Generation: Input street network CRS: {current_crs.authid()}")
    ...
    width_adjusted_streets = QgsVectorLayer(f"LineString?crs={current_crs.authid()}",
                                            "width_adjusted_streets_temp", "memory")
    ...
    if width_adjusted_streets.fields().lookupField(widths_fieldname) == -1:
        width_field = QgsField(widths_fieldname, QVariant.Double) # Assuming width is double
    ...
    features_to_add = []
    if check_building_overlap and buildings_layer and buildings_layer.featureCount() > 0:
        ...
        dissolved_buildings = dissolve_tosinglegeom(buildings_layer) # Assumes buildings_layer is valid
        ...
        for street_feat in street_network_layer.getFeatures():
            if feedback.isCanceled(): return None, None, None, None
            ...
            if original_width_val is None: original_width_val = 0.0 # Handle NULL widths

            try:
                current_street_width = float(original_width_val)
            except (ValueError, TypeError):
                feedback.pushWarning(f"Could not parse width '{original_width_val}' for street FID {street_feat.id()}. Using 0.0.")
                current_street_width = 0.0

            d_to_nearest_building = street_feat.geometry().distance(dissolved_buildings_geom)
            ...
            effective_sidewalk_projection_one_side = (current_street_width / 2.0) + (added_width_for_sidewalk_axis_total / 2.0)

            diff_dist = (d_to_nearest_building - min_dist_to_building) - effective_sidewalk_projection_one_side
            ...
            if diff_dist < 0:
                ...
                if adjusted_street_width < min_generated_width_near_building: # This might be wrong.
        ...
        if features_to_add: width_adjusted_streets_dp.addFeatures(features_to_add)
        feedback.pushInfo(f"Street widths adjusted. Count: {width_adjusted_streets.featureCount()}")

    else: # No building overlap check or no buildings
        feedback.pushInfo("Skipping building overlap checks for sidewalk width adjustment.")
        ...
        for street_feat in street_network_layer.getFeatures():
            ...
            original_width_val = street_feat.attribute(widths_fieldname)
            if original_width_val is None: original_width_val = 0.0
            try: current_street_width = float(original_width_val)
            except: current_street_width = 0.0
    ...
    buffer_distance_expression = f'("{widths_fieldname}" / 2) + {added_width_for_sidewalk_axis_total / 2.0}'

    proto_undissolved_buffer = generate_buffer(width_adjusted_streets, buffer_distance_expression, dissolve=False)
    if not proto_undissolved_buffer: raise QgsProcessingException("Failed at proto_undissolved_buffer generation.")

    dissolved_once_buffer = dissolve_tosinglegeom(proto_undissolved_buffer)
    if not dissolved_once_buffer: raise QgsProcessingException("Failed at first dissolve for buffer.")

    # Rounding buffers
    proto_dissolved_buffer_step2 = generate_buffer(dissolved_once_buffer, curve_radius)
    if not proto_dissolved_buffer_step2: raise QgsProcessingException("Failed at curve_radius buffer generation.")

    dissolved_sidewalk_area_polygons = generate_buffer(proto_dissolved_buffer_step2, -curve_radius)
    if not dissolved_sidewalk_area_polygons: raise QgsProcessingException("Failed at negative curve_radius buffer generation.")
    ...
    big_temp_buffer_for_diff = generate_buffer(dissolved_sidewalk_area_polygons, big_buffer_d) # Outer extent
    if not big_temp_buffer_for_diff: raise QgsProcessingException("Failed to create big_temp_buffer_for_diff.")

    # This diff_layer contains the "donut" polygons representing sidewalks
    sidewalk_polygons_as_donuts = compute_difference_layer(big_temp_buffer_for_diff, dissolved_sidewalk_area_polygons)
    if not sidewalk_polygons_as_donuts: raise QgsProcessingException("Failed at compute_difference_layer for sidewalk donuts.")
    sidewalk_polygons_as_donuts.setCrs(current_crs)

    sidewalk_polygons_singleparts = convert_multipart_to_singleparts(sidewalk_polygons_as_donuts)
    if not sidewalk_polygons_singleparts: raise QgsProcessingException("Failed at convert_multipart_to_singleparts for sidewalk polygons.")
    ...
    for street_feat in width_adjusted_streets.getFeatures():
        ...
        try: current_street_width = float(current_street_width_val)
        except: current_street_width = 0.0 # Default if width is invalid
        ...
        tag_buffer_dist = (current_street_width / 2.0) + (added_width_for_sidewalk_axis_total / 2.0) + 0.5 # Added 0.5m margin
        ...
        if sidewalk_tag == "no" or sidewalk_both_tag == "no":
            geom_exclusion = street_geom.buffer(tag_buffer_dist, 5, Qgis.EndCapStyle.Flat, Qgis.JoinStyle.Miter, 10)
        elif sidewalk_tag == "left" or sidewalk_left_tag == "yes": # Sidewalk only on left
            geom_sure = street_geom.singleSidedBuffer(tag_buffer_dist, 5, False) # Left side
            geom_exclusion = street_geom.singleSidedBuffer(tag_buffer_dist, 5, True)  # Right side is exclusion
        elif sidewalk_tag == "right" or sidewalk_right_tag == "yes": # Sidewalk only on right
            geom_sure = street_geom.singleSidedBuffer(tag_buffer_dist, 5, True)   # Right side
            geom_exclusion = street_geom.singleSidedBuffer(tag_buffer_dist, 5, False) # Left side is exclusion
        elif sidewalk_left_tag == "no": # No sidewalk on left
            current_exclusion = street_geom.singleSidedBuffer(tag_buffer_dist, 5, False)
            geom_exclusion = current_exclusion if geom_exclusion is None else geom_exclusion.combine(current_exclusion)
            if sidewalk_right_tag == "yes": # Sidewalk on right
                 current_sure = street_geom.singleSidedBuffer(tag_buffer_dist, 5, True)
                 geom_sure = current_sure if geom_sure is None else geom_sure.combine(current_sure)
            ...
            if sidewalk_left_tag == "yes": # Sidewalk on left
                ...
                geom_sure = current_sure if geom_sure is None else geom_sure.combine(current_sure)
        elif sidewalk_tag == "both" or sidewalk_tag == "yes" or sidewalk_both_tag == "yes": # Sidewalk on both sides
            geom_sure = street_geom.buffer(tag_buffer_dist, 5, Qgis.EndCapStyle.Flat, Qgis.JoinStyle.Miter, 10)

        # Default case: if no specific sidewalk tags imply "yes" on both, assume sure zone covers full buffer
        if geom_sure is None and geom_exclusion is None: # No 'no' tags, and no explicit 'yes' tags for one side
             geom_sure = street_geom.buffer(tag_buffer_dist, 5, Qgis.EndCapStyle.Flat, Qgis.JoinStyle.Miter, 10)
    ...
    exclusion_zones_poly = layer_from_featlist(exclusion_zones_featlist, "exclusion_zones_temp", "Polygon", CRS=current_crs)
    sure_zones_poly = layer_from_featlist(sure_zones_featlist, "sure_zones_temp", "Polygon", CRS=current_crs)
    feedback.pushInfo(f"Generated {exclusion_zones_poly.featureCount()} exclusion zones, {sure_zones_poly.featureCount()} sure zones.")

    # --- 6. Apply exclusion zones to sidewalk polygons ---
    feedback.pushInfo("Applying exclusion zones to sidewalk areas...")
    sidewalk_polygons_final = sidewalk_polygons_singleparts # Start with all sidewalk polygons
    if exclusion_zones_poly.featureCount() > 0:
        ...
        if dissolved_exclusions and dissolved_exclusions.featureCount() > 0:
            sidewalk_polygons_final = compute_difference_layer(sidewalk_polygons_singleparts, dissolved_exclusions)
            if not sidewalk_polygons_final:
                feedback.pushWarning("Difference operation for exclusion zones failed. Using un-excluded sidewalks.")
                sidewalk_polygons_final = sidewalk_polygons_singleparts # Fallback
            else:
                sidewalk_polygons_final.setCrs(current_crs)
                feedback.pushInfo(f"Sidewalk areas after exclusion: {sidewalk_polygons_final.featureCount()} parts.")
        else:
            feedback.pushInfo("No valid exclusion zones to apply or dissolving them failed.")
    else:
        feedback.pushInfo("No exclusion zones generated.")

    # --- Extract final sidewalk lines ---
    whole_sidewalks_lines = extract_lines_from_polygons(sidewalk_polygons_final, "memory:whole_sidewalks_lines_algo")
    if not whole_sidewalks_lines: raise QgsProcessingException("Failed to extract final sidewalk lines.")
    whole_sidewalks_lines.setCrs(current_crs)
    feedback.pushInfo(f"Final sidewalk lines extracted: {whole_sidewalks_lines.featureCount()} features.")
    ...
    if dissolved_protoblocks_layer and dissolved_protoblocks_layer.featureCount() > 0:
        feedback.pushInfo("Filtering sidewalk lines to keep only those intersecting protoblock areas...")
        dissolved_protoblock_geom = get_first_feature_or_geom(dissolved_protoblocks_layer, True)

        # Create a new layer for filtered sidewalks
        filtered_sidewalk_lines = QgsVectorLayer(f"LineString?crs={current_crs.authid()}",
                                                 "filtered_sidewalk_lines_algo", "memory")
        filtered_sidewalk_lines_dp = filtered_sidewalk_lines.dataProvider()
        filtered_sidewalk_lines_dp.addAttributes(whole_sidewalks_lines.fields()) # Keep fields if any
        ...
        if kept_sidewalks: filtered_sidewalk_lines_dp.addFeatures(kept_sidewalks)
        feedback.pushInfo(f"Sidewalk lines filtered by protoblocks: {filtered_sidewalk_lines.featureCount()} features remain.")
        whole_sidewalks_lines = filtered_sidewalk_lines # Replace with filtered
    else:
        feedback.pushWarning("No dissolved protoblocks layer provided or it's empty; skipping filtering of sidewalks by protoblocks.")
    ...
    return whole_sidewalks_lines, exclusion_zones_poly, sure_zones_poly, width_adjusted_streets
